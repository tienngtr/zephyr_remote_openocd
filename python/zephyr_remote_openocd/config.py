...
def default_config_path() -> Path:
    ...
    return (
        ...
        if override
        else Path.home() / ".config" / "zephyr_remote_openocd" / "config.toml"
    )
...
def load_config(path: Path | None = None) -> RemoteOpenOcdConfig:
    ...
    if not all(
        isinstance(section, dict) for section in (runner, remote, ssh, openocd_section, paths)
    ):
    ...
    selected = runner.get("default", "openocd")
    if selected not in {"openocd", "remote_openocd"}:
        raise ConfigError(
            f"invalid runner.default in {config_path}: expected 'openocd' or 'remote_openocd'"
        )
