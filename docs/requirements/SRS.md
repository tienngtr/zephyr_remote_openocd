...
be detected and regenerated by the normal Zephyr build and reconfiguration
machinery for builds using the module. This SHALL be possible without a
pristine build or a full firmware compile and link.

### REQ-FUNC-SELECT-009

After a configuration change, a normal build or reconfiguration operation
SHOULD update generated runner state before a subsequent west runner command
consumes it. The runner command is not required to perform a full firmware
build solely to apply that configuration change.
...
After the configured default changes, a normal build or reconfiguration
operation updates the generated default without requiring a pristine build or
a full firmware compile and link. A subsequent west runner invocation
consumes that generated state; a runner command that suppresses regeneration
may continue to use existing state.
...
not emit the selected image's marker. It then flashes the selected image and
...
post-load reset, and stops at a configured source symbol. It reads the program
counter and current instruction, then detaches cleanly. Fresh application
console output is not required.

`west attach -r remote_openocd` connects without loading an image. GDB reads the
program counter and current instruction before detaching. `west debugserver`
with `-r remote_openocd` remains active without launching GDB and allows an
