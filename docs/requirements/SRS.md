...
`west flash -r remote_openocd` SHALL program the intended remote target.
...
Under representative local conditions, each invocation of `remote_openocd` SHOULD introduce less than 0.5 seconds of additional runner-controlled startup processing compared with the equivalent invocation of Zephyr's built-in `openocd` runner.
