...
Changes to configuration that affect generated Zephyr runner state are
detected and regenerated by the normal build and reconfiguration workflow
without requiring a pristine build or a full firmware compile and link. By
default, the configuration path is:
...
edit config.yaml
      |
      v
normal build or reconfiguration operation
      |
      v
...
west flash/debug consumes generated state
      |
      v
runner command with --no-rebuild may consume existing state
```

Normal runner commands trigger their usual pre-run build, so configuration
changes are picked up after the module is configured into the build. Explicit
`west build` is also available for setup or regeneration. Runner commands are
not required to perform a full firmware build solely to apply the configuration
change; when regeneration is explicitly suppressed, stale generated state may
remain until a later regeneration.
