...
def build_steps(args: argparse.Namespace) -> list[Step]:
    ...
    steps = [
        Step("pytest", _python_module("pytest", "-q")),
        Step("static_check", (sys.executable, "scripts/static_check.py")),
    ]
...
def local_leak_scan() -> dict[str, object]:
    ...
    for line in result.stdout.splitlines():
        ...
        if len(fields) < 2 or fields[0] == current:
            continue
        if any(token in fields[1] for token in ("remote_openocd", "helper.py")):
